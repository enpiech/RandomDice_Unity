...
namespace Plugins.AssetUsageDetector.Editor
{
    ...
    public abstract class SearchMatch
    {
        public readonly Object
            Context; // Almost always equal to Source. This is the Object that needs to be dirtied (if not null) to notify Unity of changes to Value
    ...
    public abstract class GenericSearchMatch : SearchMatch
    {
        ...
        internal GenericSearchMatch(object source, Object value, SetterFunction setter) : base(source, value)
        {
            Setter = setter;
        }

        internal GenericSearchMatch(object source, Object value, Object context, SetterFunction setter) : base(source, value, context)
        {
            Setter = setter;
        }
    ...
    public class SerializedPropertyMatch : SearchMatch
    {
        ...
        internal SerializedPropertyMatch(Object source, Object value, SerializedProperty property) : base(source, value)
        {
        ...
        protected override bool ChangeValue(Object newValue, out bool setContextDirty)
        {
            ...
            switch (SerializedProperty.propertyType)
            {
                ...
                case SerializedPropertyType.ManagedReference:
                    SerializedProperty.managedReferenceValue = newValue;
                    break;
#endif
            }
    ...
    public class ReflectionMatch : SearchMatch
    {
        ...
        internal ReflectionMatch(object source, Object value, object variable) : base(source, value)
        {
            Variable = variable;
        }
        ...
        protected override bool ChangeValue(Object newValue, out bool setContextDirty)
        {
            ...
            {
                var property = (PropertyInfo)Variable;
            ...
            {
                var dictionary = (IDictionary)Variable;
                ...
                do
                {
                    ...
                    foreach (var dictKey in dictionary.Keys)
                    {
                        ...
                        if (ReferenceEquals(dictValue, Value))
                        {
    ...
    public class AssetImporterDefaultValueMatch : SearchMatch
    {
        ...
        internal AssetImporterDefaultValueMatch(Object source, Object value, string variable, VariableGetterHolder[] monoScriptAllVariables) :
            base(source, value)
        {
            Variable = variable;
        ...
        protected override bool ChangeValue(Object newValue, out bool setContextDirty)
        {
            ...
            {
                var monoImporter = (MonoImporter)Source;

                var variableNames = new List<string>(8);
                var variableValues = new List<Object>(8);

                for (var i = 0; i < MonoScriptAllVariables.Length; i++)
                {
                    if (MonoScriptAllVariables[i].isSerializable && !MonoScriptAllVariables[i].IsProperty)
                    {
                        var variableDefaultValue = monoImporter.GetDefaultReference(MonoScriptAllVariables[i].Name);
                ...
                EditorApplication.delayCall +=
                    () => AssetDatabase.ImportAsset(monoImporter
                        .assetPath); // If code recompiles during search, it will break the search. Give it a 1 frame delay
            }
            else if (Source is ShaderImporter)
            {
                var shaderImporter = (ShaderImporter)Source;
                var shader = shaderImporter.GetShader();

                var textureNames = new List<string>(16);
                var textureValues = new List<Texture>(16);
#if UNITY_2018_1_OR_NEWER
                var nonModifiableTextureNames = new List<string>(16);
                var nonModifiableTextureValues = new List<Texture>(16);
#endif
                ...
                for (var i = 0; i < shaderPropertyCount; i++)
                {
                    ...
                    {
                        var propertyDefaultValue = shaderImporter.GetNonModifiableTexture(propertyName);
...
#endif
                    {
                        var propertyDefaultValue = shaderImporter.GetDefaultTexture(propertyName);
                        if (propertyDefaultValue == Value && propertyName == Variable)
                        {
                            propertyDefaultValue = (Texture)newValue;
                        }
    ...
    /// <summary>
    ///     - Source: Animation, Animator, AnimatorStateMachine, AnimatorState, AnimatorControllerLayer, BlendTree,
    ///     PlayableDirector* or AnimationClip*
    ///     - Context: If Source is AnimatorControllerLayer, then its RuntimeAnimatorController. Otherwise, equal to Source
    ///     - Value: AnimationClip, AnimatorController or AvatarMask used in Source (*for PlayableDirector and AnimationClip,
    ///     it can be any Object value)
    /// </summary>
    public class AnimationSystemMatch : GenericSearchMatch
    {
        ...
        internal AnimationSystemMatch(object source, Object value, Object context, SetterFunction setter) : base(source, value, context,
            setter)
        {
        }
    }
    ...
    public class BehaviourUsageMatch : ReadOnlySearchMatch
    {
        ...
        internal BehaviourUsageMatch(Object source, MonoScript value, Object behaviour) : base(source, value)
        {
            Behaviour = behaviour;
        }
    }
