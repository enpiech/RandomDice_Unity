...
namespace Plugins.AssetUsageDetector.Editor
{
    ...
    public class AssetUsageDetectorWindow : EditorWindow, IHasCustomMenu
    {
        ...
        private static readonly GUIContent windowTitle = new("Asset Usage Detector");
        private static readonly Vector2 windowMinSize = new(325f, 220f);

        private readonly AssetUsageDetector core = new();
        private readonly List<Object> excludedAssets = new() { null }; // These assets won't be searched for references
        private readonly ObjectListDrawer excludedAssetsDrawer = new("Don't search following asset(s):", false);
        private readonly List<Object> excludedScenes = new() { null }; // These scenes won't be searched for references
        private readonly ObjectListDrawer excludedScenesDrawer = new("Don't search in following scene(s):", false);

        private readonly GUILayoutOption GL_WIDTH_12 = GUILayout.Width(12f);

        // This isn't readonly so that it can be serialized
        private readonly List<ObjectToSearch> objectsToSearch = new() { new(null) };

        private readonly ObjectToSearchListDrawer objectsToSearchDrawer = new();

        private readonly List<Object> searchInAssetsSubset = new() { null }; // If not empty, only these assets are searched for references
        private readonly ObjectListDrawer searchInAssetsSubsetDrawer = new("Search following asset(s) only:", false);
        private bool calculateUnusedObjects;
        ...
        private bool noAssetDatabaseChanges;
        ...
        private SearchRefactoring searchRefactoring; // Its value can be assigned via ShowAndSearch
        ...
        private void OnGUI()
        {
            ...
            else if (currentPhase == Phase.Setup)
            {
                ...
                dontSearchInSourceAssets =
                    WordWrappingToggleLeft("Don't search \"SEARCHED OBJECTS\" themselves for references", dontSearchInSourceAssets);
                ...
                if (!EditorApplication.isPlaying)
                {
                    ...
                    if (searchInScenesInBuild)
                    {
                        ...
                        searchInScenesInBuildTickedOnly =
                            EditorGUILayout.ToggleLeft("Ticked only", searchInScenesInBuildTickedOnly, Utilities.GL_WIDTH_100);
                        searchInScenesInBuildTickedOnly =
                            !EditorGUILayout.ToggleLeft("All", !searchInScenesInBuildTickedOnly, Utilities.GL_WIDTH_100);
                ...
                searchInProjectSettings =
                    WordWrappingToggleLeft("Project Settings (Player Settings, Graphics Settings etc.)", searchInProjectSettings);
                ...
                hideReduntantPrefabVariantLinks =
                    WordWrappingToggleLeft(
                        ...
                        hideReduntantPrefabVariantLinks);
#endif
                noAssetDatabaseChanges = WordWrappingToggleLeft("I haven't modified any assets/scenes since the last search (faster search)",
                    noAssetDatabaseChanges);
                showDetailedProgressBar = WordWrappingToggleLeft("Update search progress bar more often (cancelable search) (slower search)",
            ...
            else if (currentPhase == Phase.Complete)
            {
                ...
                {
                    noAssetDatabaseChanges =
                        WordWrappingToggleLeft("I haven't modified any assets/scenes since the last search (faster Refresh)",
                            noAssetDatabaseChanges);
        ...
        void IHasCustomMenu.AddItemsToMenu(GenericMenu contextMenu)
        {
            ...
            contextMenu.AddItem(new GUIContent("Settings"), false,
                () => SettingsService.OpenProjectSettings("Project/yasirkula/Asset Usage Detector"));
#else
			contextMenu.AddItem( new GUIContent( "Settings" ), false, () =>
			{
				...
				System.Collections.IList sections =
 (System.Collections.IList) preferencesWindowType.GetField( "m_Sections", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance ).GetValue( preferencesWindow );
            ...
            if (currentPhase == Phase.Setup)
            {
                ...
                {
                    for (var i = objectsToSearch.Count - 1; i >= 0; i--)
                    {
        ...
        {
            var windows = Resources.FindObjectsOfTypeAll<AssetUsageDetectorWindow>();
            var window = Array.Find(windows, w => w && !w.IsLocked);
            if (!window)
            {
                window = Array.Find(windows, w => w);
            }
        ...
        private void ShowAndSearchInternal(IEnumerable<Object> searchObjects, AssetUsageDetector.Parameters searchParameters,
        ...
        {
            var sceneSearchMode = SceneSearchMode.None;
        ...
        private void ParseSceneSearchMode(SceneSearchMode sceneSearchMode)
        {
            ...
            searchInScenesInBuild = (sceneSearchMode & SceneSearchMode.ScenesInBuildSettingsAll) == SceneSearchMode.ScenesInBuildSettingsAll ||
                                    ...
                                    SceneSearchMode.ScenesInBuildSettingsTickedOnly;
            searchInScenesInBuildTickedOnly =
                (sceneSearchMode & SceneSearchMode.ScenesInBuildSettingsAll) != SceneSearchMode.ScenesInBuildSettingsAll;
        ...
        private void DrawObjectsToSearchSection()
        {
            ...
            var searchedObjectsHeaderRect = GUILayoutUtility.GetLastRect();
        ...
        {
            GUILayout.BeginHorizontal();
            var result = EditorGUILayout.ToggleLeft(GUIContent.none, value, GL_WIDTH_12);
        ...
        public void ReplacePrefabStageObjectsWithAssets(PrefabStage prefabStage)
        {
            ...
            for (var i = 0; i < objectsToSearch.Count; i++)
            {
                var obj = objectsToSearch[i].obj;
                if (obj != null && !obj.Equals(null) && obj is GameObject && prefabStage.IsPartOfPrefabContents((GameObject)obj))
                {
                    var prefabStageObjectSource = ((GameObject)obj).FollowSymmetricHierarchy(prefabStage.prefabContentsRoot, prefabAsset);
                    if (prefabStageObjectSource != null)
                    {
                        objectsToSearch[i].obj = prefabStageObjectSource;
                    }

                    var subAssets = objectsToSearch[i].subAssets;
                    for (var j = 0; j < subAssets.Count; j++)
                    {
        ...
        internal void OnSettingsChanged(bool highlightedSearchTextColorChanged = false, bool tooltipDescriptionsColorChanged = false)
        {
            ...
            for (var i = searchResult.NumberOfGroups - 1; i >= 0; i--)
            {
