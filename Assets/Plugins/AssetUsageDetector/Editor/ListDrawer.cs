...
namespace Plugins.AssetUsageDetector.Editor
{
    public abstract class ListDrawer<T>
    {
        ...
        public bool Draw(List<T> list)
        {
            ...
            for (var i = 0; i < list.Count; i++)
            {
                ...
                var prevObject = GetObjectFromElement(element);
                var newObject = EditorGUILayout.ObjectField("", prevObject, typeof(Object), acceptSceneObjects);
    ...
    public class ObjectToSearchListDrawer : ListDrawer<ObjectToSearch>
    {
        ...
        {
            var subAssetsToSearch = element.subAssets;
            if (subAssetsToSearch.Count > 0)
            {
                GUILayout.BeginHorizontal();

                // 0-> all toggles off, 1-> mixed, 2-> all toggles on
                var toggleAllSubAssets = subAssetsToSearch[0].shouldSearch;
                var mixedToggle = false;
                for (var j = 1; j < subAssetsToSearch.Count; j++)
                {
                ...
                if (GUI.changed)
                {
                    for (var j = 0; j < subAssetsToSearch.Count; j++)
                    {
