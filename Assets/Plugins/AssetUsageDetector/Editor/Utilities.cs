...
namespace Plugins.AssetUsageDetector.Editor
{
    public static class Utilities
    {
        ...
        private static readonly HashSet<Type> primitiveUnityTypes = new()
        {
        ...
        private static readonly HashSet<string> folderContentsSet = new();

        internal static readonly StringBuilder stringBuilder = new(400);
        ...
        {
            var folderContents = AssetDatabase.FindAssets("", new[] { AssetDatabase.GetAssetPath(folderAsset) });
            ...
            {
                var filePath = AssetDatabase.GUIDToAssetPath(folderContents[i]);
        ...
        public static void GetObjectsToSelectAndPing(this Object obj, out Object selection, out Object pingTarget)
        {
            ...
            if (obj.IsAsset())
            {
                if (obj is GameObject)
                {
                    ...
                    var objTR = ((GameObject)obj).transform.root;

                    var prefabAssetType = PrefabUtility.GetPrefabAssetType(objTR.gameObject);
                    if (prefabAssetType == PrefabAssetType.Regular || prefabAssetType == PrefabAssetType.Variant)
                    {
                        var assetPath = AssetDatabase.GetAssetPath(objTR.gameObject);
...
#endif
                        {
                            var prefabStageGO = FollowSymmetricHierarchy((GameObject)obj, ((GameObject)obj).transform.root.gameObject,
                                openPrefabStage.prefabContentsRoot);
        ...
        public static GameObject FollowSymmetricHierarchy(this GameObject go, GameObject thisRoot, GameObject symmetricRoot)
        {
            ...
            var root1 = thisRoot.transform;
            var root2 = symmetricRoot.transform;
            while (root1 != target)
            {
                var temp = target;
                ...
                Transform newRoot2;
                var siblingIndex = temp.GetSiblingIndex();
        ...
        public static Rect GetScreenFittedRect(Rect originalRect)
        {
            ...
            {
                screenFittedRectGetter = typeof(EditorWindow).Assembly.GetType("UnityEditor.ContainerWindow")
                    .GetMethod("FitRectToScreen", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static);
            }
