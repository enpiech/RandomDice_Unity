...
namespace Plugins.AssetUsageDetector.Editor
{
    ...
    public enum SceneSearchMode
    {
        None = 0,
        OpenScenes = 1,
        ScenesInBuildSettingsAll = 2,
        ScenesInBuildSettingsTickedOnly = 4,
        AllScenes = 8
    }
    ...
    public partial class AssetUsageDetector
    {
        ...
        private readonly HashSet<string> alwaysSearchedExtensionsSet = new();

        // assetsToSearchSet's path(s)
        private readonly HashSet<string> assetsToSearchPathsSet = new();

        // The root prefab objects in assetsToSearchSet that will be used to search for prefab references
        private readonly List<GameObject> assetsToSearchRootPrefabs = new(4);

        // Project asset(s) in objectsToSearchSet
        private readonly HashSet<Object> assetsToSearchSet = new();
        ...
        private readonly List<object> callStack = new(64);

        // Path(s) of the assets that should be excluded from the search
        private readonly HashSet<string> excludedAssetsPathsSet = new();

        private readonly List<ReferenceNode> nodesPool = new(32);

        // A set that contains the searched scene object(s), asset(s) and their sub-assets (if any)
        private readonly HashSet<Object> objectsToSearchSet = new();

        // Scenes of scene object(s) in objectsToSearchSet
        private readonly HashSet<string> sceneObjectsToSearchScenesSet = new();

        // An optimization to search an object only once (key is a hash of the searched object)
        private readonly Dictionary<string, ReferenceNode> searchedObjects = new(4096);

        private readonly Dictionary<int, ReferenceNode>
            searchedUnityObjects = new(32768); // Unity objects use their instanceIDs as key which is more performant
        ...
        public SearchResult Run(Parameters searchParameters)
        {
            ...
            if (openPrefabStage != null)
            {
                ...
                else
                {
                    ...
                    var prefabAssetPath = openPrefabStage.assetPath;
#else
            ...
            if (!isInPlayMode && !Utilities.AreScenesSaved() && !EditorUtility.DisplayDialog("Asset Usage Detector",
                    "Some scene(s) aren't saved. This may result in incorrect search results in those scene(s). Proceed?", "Yes", "Cancel"))
            {
            ...
            var initialSceneSetup = !isInPlayMode ? EditorSceneManager.GetSceneManagerSetup() : null;
            ...
            try
            {
                ...
                var folderContentsSet = new HashSet<string>();
                foreach (var obj in searchParameters.objectsToSearch)
                {
                ...
                foreach (var filePath in folderContentsSet)
                {
                    // Skip scene assets
                ...
                var projectSettingsToSearch = new string[0];
                if (searchParameters.searchInProjectSettings && assetsToSearchSet.Count > 0)
                {
                    var projectSettingsAssets = Directory.GetFiles("ProjectSettings");
                    projectSettingsToSearch = new string[projectSettingsAssets.Length];
                    for (var i = 0; i < projectSettingsAssets.Length; i++)
                    {
                ...
                var scenesToSearch = new HashSet<string>();
                if (searchParameters.searchInScenesSubset != null && searchParameters.searchInScenesSubset.Length > 0)
                {
                    foreach (var obj in searchParameters.searchInScenesSubset)
                    {
                        if (obj == null || obj.Equals(null))
                        {
                            continue;
                        ...
                        if (obj.IsFolder())
                        {
                            var folderContents = AssetDatabase.FindAssets("t:SceneAsset", new[] { AssetDatabase.GetAssetPath(obj) });
                ...
                else
                {
                    ...
                    var searchInScenesInBuildTickedAll = (searchParameters.searchInScenes & SceneSearchMode.ScenesInBuildSettingsAll) ==
                                                         SceneSearchMode.ScenesInBuildSettingsAll;
                    if (searchInScenesInBuildTickedAll ||
                        (searchParameters.searchInScenes & SceneSearchMode.ScenesInBuildSettingsTickedOnly) ==
                ...
                if (isInPlayMode)
                {
                    ...
                    var skippedScenes = new List<string>(scenesToSearch.Count);
                    ...
                    if (skippedScenes.Count > 0)
                    {
                        ...
                        sb.Length = 0;
                        sb.Append("Can't search unloaded scenes while in play mode, skipped ").Append(skippedScenes.Count)
                            .AppendLine(" scene(s):");
                        for (var i = 0; i < skippedScenes.Count; i++)
                        {
                ...
                var searchTotalProgress = scenesToSearch.Count;
                ...
                if (searchParameters.searchInAssetsFolder && assetsToSearchSet.Count > 0)
                {
                    ...
                    else
                    {
                        ...
                        foreach (var obj in searchParameters.searchInAssetsSubset)
                        {
                            if (obj == null || obj.Equals(null))
                            {
                                continue;
                    ...
                    {
                        foreach (var obj in searchParameters.excludedAssetsFromSearch)
                        {
                            if (obj == null || obj.Equals(null))
                            {
                                continue;
                    ...
                    {
                        if (searchParameters.showDetailedProgressBar && ++searchProgress % 30 == 1 &&
                            EditorUtility.DisplayCancelableProgressBar("Please wait...", "Searching assets",
                                (float)searchProgress / searchTotalProgress))
                        {
                            throw new Exception("Search aborted");
                        }
                ...
                if (projectSettingsToSearch.Length > 0)
                {
                    ...
                    if (EditorUtility.DisplayCancelableProgressBar("Please wait...", "Searching Project Settings",
                            (float)searchProgress / searchTotalProgress))
                    {
                        throw new Exception("Search aborted");
                    }

                    for (var i = 0; i < projectSettingsToSearch.Length; i++)
                    {
                        if (searchParameters.showDetailedProgressBar && ++searchProgress % 30 == 1 &&
                            EditorUtility.DisplayCancelableProgressBar("Please wait...", "Searching Project Settings",
                                (float)searchProgress / searchTotalProgress))
                        {
                            throw new Exception("Search aborted");
                        }

                        var assets = AssetDatabase.LoadAllAssetsAtPath(projectSettingsToSearch[i]);
                        if (assets != null && assets.Length > 0)
                        {
                ...
                if (scenesToSearch.Count > 0)
                {
                    ...
                    var excludedScenesPathsSet = new HashSet<string>();
                    if (searchParameters.excludedScenesFromSearch != null)
                    {
                        foreach (var obj in searchParameters.excludedScenesFromSearch)
                        {
                            if (obj == null || obj.Equals(null))
                            {
                                continue;
                    ...
                    {
                        if (EditorUtility.DisplayCancelableProgressBar("Please wait...", "Searching scene: " + scenePath,
                                (float)++searchProgress / searchTotalProgress))
                        {
                            throw new Exception("Search aborted");
                        }
                ...
                if (isInPlayMode && searchParameters.searchInScenes != SceneSearchMode.None)
                {
                    ...
                    var rootGameObjects = GetDontDestroyOnLoadObjects();
                ...
                Debug.Log("Searched " + searchedObjectsCount + " objects in " +
                          (EditorApplication.timeSinceStartup - searchStartTime).ToString("F2") + " seconds");
            ...
            finally
            {
                ...
                {
#if UNITY_2020_1_OR_NEWER
                    var shouldOpenPrefabStageWithoutContext = true;
                    if (openPrefabStageContextObject != null && !openPrefabStageContextObject.Equals(null))
                    {
                        try
                        {
                            ...
                            var prefabStageOpenerWithContext = typeof(PrefabStageUtility).GetMethod("OpenPrefab",
                                BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static, null,
                                new Type[2] { typeof(string), typeof(GameObject) }, null);
                            if (prefabStageOpenerWithContext != null)
                            {
                                prefabStageOpenerWithContext.Invoke(null,
                                    new object[2] { openPrefabStageAssetPath, openPrefabStageContextObject });
        ...
        {
            currentSearchResultGroup = new SearchResultGroup("Unused Objects", SearchResultGroup.GroupType.UnusedObjects, false);

            usedObjectsSet = new HashSet<Object>();
            var usedObjectPathsSet =
                new HashSet<string>(); // For assets: stores the filepaths, For scene objects: stores the topmost GameObject's instanceID
            foreach (var searchResultGroup in searchResult)
            {
                for (var j = 0; j < searchResultGroup.NumberOfReferences; j++)
                {
                    var obj = searchResultGroup[j].UnityObject;
                    if (obj is Component)
                    {
                    ...
                    if (usedObjectsSet.Add(obj))
                    {
                        ...
                        {
                            for (var parent = ((GameObject)obj).transform.parent; parent != null; parent = parent.parent)
                            {
            ...
            var unusedMainObjectNodes = new Dictionary<string, ReferenceNode>(objectsToSearchSet.Count - usedObjectsSet.Count);
            var unusedSubObjectNodes = new Dictionary<string, List<ReferenceNode>>(objectsToSearchSet.Count - usedObjectsSet.Count);
            foreach (var obj in objectsToSearchSet)
            {
                ...
                if (obj is GameObject)
                {
                    ...
                    if (searchedTopmostGameObject &&
                        !string.IsNullOrEmpty(assetPath)) // Omit GameObject assets if their parent objects are already included in search
                    {
                        continue;
                    }
                }
                ...
                if (obj is Mesh && !string.IsNullOrEmpty(assetPath) && AssetDatabase.GetMainAssetTypeAtPath(assetPath) == typeof(GameObject) &&
                    objectsToSearchSet.Contains(AssetDatabase.LoadMainAssetAtPath(assetPath)))
                {
                    continue;
                }
                ...
                if (string.IsNullOrEmpty(assetPath))
                {
                    ...
                    {
                        var dictionaryKey = searchedTopmostGameObject.GetHashCode().ToString();
            ...
            foreach (var kvPair in unusedMainObjectNodes)
            {
                ...
                if (unusedSubObjectNodes.TryGetValue(kvPair.Key, out unusedSubAssetNodesAtPath))
                {
                    ...
                    for (var i = 0; i < unusedSubAssetNodesAtPath.Count; i++)
                    {
                    ...
                    (!usedObjectPathsSet
                        .Contains(kvPair
                            .Key)) // If a main asset has sub-assets and all of them are used, consider the main asset as used, as well (especially useful for Sprite assets)
                {
                    currentSearchResultGroup.AddReference(kvPair.Value);
                }
            ...
            foreach (var kvPair in
                     unusedSubObjectNodes) // These aren't linked to any unusedMainObjectNodes, add them as root nodes to the search result group
            {
            ...
            if (currentSearchResultGroup.NumberOfReferences > 0)
            {
                for (var i = 0; i < currentSearchResultGroup.NumberOfReferences; i++)
                {
        ...
        private void AddSearchedObjectToFilteredSets(Object obj, bool expandGameObjects)
        {
            ...
            if (openPrefabStage != null && openPrefabStagePrefabAsset != null && obj is GameObject &&
                openPrefabStage.IsPartOfPrefabContents((GameObject)obj))
            {
                var prefabStageObjectSource =
                    ((GameObject)obj).FollowSymmetricHierarchy(openPrefabStage.prefabContentsRoot, openPrefabStagePrefabAsset);
            ...
            if (isAsset)
            {
                ...
                if (go != null)
                {
                    ...
                    var shouldAddRootPrefabEntry = true;
                    for (var i = assetsToSearchRootPrefabs.Count - 1; i >= 0; i--)
                    {
                        var rootTransform = assetsToSearchRootPrefabs[i].transform;
        ...
        private void SearchScene(string scenePath, List<SearchResultGroup> searchResult, bool lazySearch, SceneSetup[] initialSceneSetup)
        {
            ...
            var canContainSceneObjectReference = scene.isLoaded &&
                                                 (!EditorSceneManager.preventCrossSceneReferences ||
                                                  sceneObjectsToSearchScenesSet.Contains(scenePath));
            if (!canContainSceneObjectReference)
            {
                var canContainAssetReference = assetsToSearchSet.Count > 0 && (isInPlayMode || AssetHasAnyReference(scenePath));
        ...
        private void BeginSearchObject(Object obj)
        {
            ...
            var searchResult = SearchObject(obj);
        ...
        private ReferenceNode SearchObject(object obj)
        {
            ...
            var searchingSourceAsset = searchingSourceAssets && ReferenceEquals(currentSearchedObject, obj);
            ...
            if (unityObject != null)
            {
                ...
                if (assetPath != null && extensionToSearchFunction.TryGetValue(Utilities.GetFileExtension(assetPath), out searchFunction) &&
                    AssetDatabase.IsMainAsset(unityObject))
                {
                ...
                if (assetPath != null && unityObject is GameObject && AssetDatabase.IsMainAsset(unityObject))
                {
                    ...
                    var prefabGameObject = (GameObject)unityObject;
                    var prefabChildren = prefabGameObject.GetComponentsInChildren<Transform>(true);
        ...
        private ReferenceNode GetReferenceNode(object nodeObject)
        {
            ...
            if (nodeObject is Object)
            {
                var hash = nodeObject.GetHashCode();
            ...
            {
                var hash = GetNodeObjectHash(nodeObject);
            ...
            if (callStack.Count > 0)
            {
                ...
                for (var i = callStack.Count - 1; i >= 0; i--)
                {
                    ...
                    if (latestUnityObjectInCallStack)
                    {
                        ...
                        {
                            var scenePath = AssetDatabase.GetAssetOrScenePath(latestUnityObjectInCallStack);
                ...
                for (var i = callStack.Count - 1; i >= 0; i--)
                {
                    ...
                    var unityObject = callStack[i] as Object;
                    if (unityObject)
                    {
        ...
        {
            public Object[] objectsToSearch;

            public SceneSearchMode searchInScenes = SceneSearchMode.AllScenes;
            public Object[] searchInScenesSubset;
            public Object[] excludedScenesFromSearch;
            public bool searchInAssetsFolder = true;
            public Object[] searchInAssetsSubset;
            public Object[] excludedAssetsFromSearch;
            ...
            public bool calculateUnusedObjects;
