...
namespace Dice
{
    public class DiceManager : MonoBehaviour
    {
        ...
        private readonly float firstPosX = -1.1f; // 첫 주사위의 X 좌표
        private readonly float firstPosY = -0.7f; // 첫 주사위의 Y 좌표
        private readonly float marginX = 0.55f; // X 간격
        private readonly float marginY = 0.55f; // Y 간격
        ...
        private void Start()
        {
            ...
            var dbConnector = diceDataBaseConnector.GetComponent<DiceDatabaseConnector>();
        ...
        {
            var diceSize = diceFieldArray.Length;
            var diceIndex = index;
            var nullIndexList = new List<int>();

            // 주사위 배열에서 null인 인덱스만 별도의 리스트에 저장
            for (var i = 0; i < diceFieldArray.Length; i++)
            {
            ...
            var newDicePosX = marginX * (diceIndex % 5) + firstPosX;
            var newDicePosY = -marginY * (diceIndex / 5) + firstPosY;
        ...
        {
            var diceSpritesScript = GetComponent<DiceSprites>();
