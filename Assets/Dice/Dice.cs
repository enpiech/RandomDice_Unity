...
namespace Dice
{
    public class Dice : MonoBehaviour
    {
        ...
        private float attackCooltime; // 공격 주기(초)
        ...
        private void Update()
        {
            ...
            if (attackCooltime >= diceStruct.attackSpeed / diceStruct.level)
            {
                // (공격속도 / 눈금 수) 마다 한번씩 공격
                attack();
                attackCooltime = 0.0f;
        ...
        private void attack()
        {
            var bullet = Instantiate(bulletPrefab, bulletPoint.gameObject.transform.position, Quaternion.identity); // 투사체 생성
            bullet.GetComponent<Bullet.Bullet>().diceStruct = diceStruct; // 주사위 정보 전달
            bullet.GetComponent<Bullet.Bullet>().targetIndex = 0; // 타겟 인덱스 전달
            if (diceStruct.target == "random")
            {
                // 랜덤일 경우의 타겟 인덱스 전달
                var monsterCount = monsterManager.GetComponent<MonsterManager>()._monsterList.Count;
                bullet.GetComponent<Bullet.Bullet>().targetIndex = Random.Range(0, monsterCount);
            }
        }
        ...
        {
            var diceManagerScript = diceManager.GetComponent<DiceManager>();
            var gameObjectIndex = Array.IndexOf(diceManagerScript.diceFieldArray, gameObject);
        ...
        private void combine(Collider2D other)
        {
            ...
            var gameObjectIndex = Array.IndexOf(diceManagerScript.diceFieldArray, gameObject);
            var otherGameObjectIndex = Array.IndexOf(diceManagerScript.diceFieldArray, other.gameObject);

            // other 주사위의 구조체 구하기
            var otherDiceStruct = other.GetComponent<Dice>().diceStruct;
            ...
            // 조커 주사위
            if (diceStruct.name == "조커" && otherDiceStruct.name != "조커")
            {
                // 둘 다 조커 주사위면 복사 없이 합성
                // 조커 주사위 제거
                diceManagerScript.diceFieldArray[gameObjectIndex] = null;
                Destroy(gameObject);
                ...
                for (var i = 0; i < 5; i++)
                {
                    var deck = GameObject.Find("PowerUpButton" + i).GetComponent<PowerUpButton>();
